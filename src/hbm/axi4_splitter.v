...
//
// 30-Jun-26  DWW     2  Now passing RID between master and slave sides
...
(
    ...
    parameter  W_FIFO_DEPTH = 512,
    parameter  R_FIFO_DEPTH = 512,
    parameter[63:0] ADDR_OFFSET = 64'h0,  
...
wire[OAW-1:0] awaddr[0:1], araddr[0:1];
assign awaddr[0] = ((S_AXI_AWADDR - ADDR_OFFSET)/2) + BASE_ADDR0;
assign awaddr[1] = ((S_AXI_AWADDR - ADDR_OFFSET)/2) + BASE_ADDR1;
assign araddr[0] = ((S_AXI_ARADDR - ADDR_OFFSET)/2) + BASE_ADDR0;
assign araddr[1] = ((S_AXI_ARADDR - ADDR_OFFSET)/2) + BASE_ADDR1;
...
for (i=0; i<2; i=i+1) begin
   always @(posedge clk) begin
      if (resetn == 0)
         ...
         b_credits[i] <= b_credits[i] + add_credit[i] - sub_credit;
   end
end
//=============================================================================
...
(
   ...
   .s_axis_tdata (S_AXI_AWLEN        ),
   .s_axis_tuser (awaddr[0]          ),
   .s_axis_tvalid(awfifo_in_tvalid   ),
   .s_axis_tready(awfifo_in_tready[0]),

    // The output bus of the FIFO
   .m_axis_tdata (M0_AXI_AWLEN  ),
   .m_axis_tuser (M0_AXI_AWADDR ),
...
(
   ...
   .s_aresetn(resetn),
    ...
    // The input bus to the FIFO
   .s_axis_tdata (S_AXI_AWLEN        ),
   .s_axis_tuser (awaddr[1]          ),
   .s_axis_tvalid(awfifo_in_tvalid   ),
   .s_axis_tready(awfifo_in_tready[1]),

    // The output bus of the FIFO
   .m_axis_tdata (M1_AXI_AWLEN  ),
   .m_axis_tuser (M1_AXI_AWADDR ),
...
(
   ...
   .s_aresetn(resetn),
    ...
    // The input bus to the FIFO
   .s_axis_tdata (S_AXI_ARLEN        ),
   .s_axis_tuser (araddr[0]          ),
   .s_axis_tvalid(arfifo_in_tvalid   ),
   .s_axis_tready(arfifo_in_tready[0]),

    // The output bus of the FIFO
   .m_axis_tdata (M0_AXI_ARLEN  ),
   .m_axis_tuser (M0_AXI_ARADDR ),
...
(
   ...
   .s_aresetn(resetn),
    ...
    // The input bus to the FIFO
   .s_axis_tdata (S_AXI_ARLEN        ),
   .s_axis_tuser (araddr[1]          ),
   .s_axis_tvalid(arfifo_in_tvalid   ),
   .s_axis_tready(arfifo_in_tready[1]),

    // The output bus of the FIFO
   .m_axis_tdata (M1_AXI_ARLEN  ),
   .m_axis_tuser (M1_AXI_ARADDR ),
...
//
// This R-channel FIFO carries the RID and RLAST signals, the other R-channel
// FIFO does not
//=============================================================================
xpm_fifo_axis #
(
